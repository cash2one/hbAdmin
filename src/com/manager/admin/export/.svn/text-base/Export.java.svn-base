...
public class Export {
	...
	public static final int EXPNUM=5000;
	...
	public Export(String xlsFileName){
	 ...
	 * @param keys 导出列map的key数组
	 * @param titleobj 列表标题数组
	 * @param request
	 * @param response
	 * @return
	 * @throws Exception
	 */
	public int exportXLS(List<Map<String,String>> resultlist,String[] keys, Object[] titleobj,
		...
		try {
			...
			for (; j < num; j++) {
				...
				Map<String,String> m=resultlist.get(j);
				for(String key:keys){
					...
					if(values!=null){
						cell.setCellValue(String.valueOf(values));
					}else{
						cell.setCellValue(" ");
					}
				}
