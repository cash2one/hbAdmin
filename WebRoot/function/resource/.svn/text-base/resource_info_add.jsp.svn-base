...
<%
	...
	String more_resource_type="";
	if(request.getAttribute("more_resource_type")!=null){
		more_resource_type=(String)request.getAttribute("more_resource_type");
	}
%>
...
<html>
  ...
  <script type="text/javascript" src="<%=basePath %>js/jquery.uploadify.v2.1.0.js"></script>
   <style>
  .listyle{
  	float:left;width:150px;padding-right:20px;list-style:none;clear:none; 
  }
  </style>
  ...
  $(function () {
        $("#uploadify1").uploadify({
		  ...
		  $("#uploadify2").uploadify({
		   ...
		   'sizeLimit'      : 19871202, //设置单个文件大小限制，单位为byte  
		   'queueSizeLimit' : 10000, //限制在一次队列中的次数（可选定几个文件）。默认值= 999，而一次可传几个文件有 simUploadLimit属性决定。
  ...
  function add(){
       ...
       /* 
		...
		$.ajax({
             type: "post",
             url: "<%=basePath%>resourceinfo/doinsertresourceinfo",
             data: {
  ...
  function sel(resource_id){
  		window.location.href="<%=basePath%>resourceinfo/doresourceinfolist?resource_id="+resource_id;
  }
  ...
  <!-- 页面位置 -->
    ...
    <div class="rightinfo">
   	<div class="itab">
	    ...
	    <li><a class="selected">${resource.type_name } 资源详情添加</a></li> 
	  	</ul>
	</div> 
	<!-- 5-游戏、4-绘本、3-动画、2-视频、1-音频-->
	<div id="tab1" class="tabson">
	    <ul class="forminfo">
		    ...
		    <select id="resource_type" name="resource_type" class="select select151" onchange="changeType()" <c:if test="${more_resource_type!=null && more_resource_type!='' && more_resource_type!='undefined'}">disabled</c:if>>
		    	<!-- 
				...
				<c:if test="${type_id=='4' || type_id=='5'}"><option value="3">图片</option></c:if>
		    	 -->
		    	<c:if test="${type_id=='1'}"><option value="1" <c:if test="${more_resource_type=='1'}">selected="selected"</c:if>>音频</option></c:if>
				<c:if test="${type_id!='5'}"><option value="2" <c:if test="${more_resource_type=='2'}">selected="selected"</c:if>>视频</option></c:if>
				<c:if test="${type_id=='6'}"><option value="3" <c:if test="${more_resource_type=='3'}">selected="selected"</c:if>>图片</option></c:if>
		    	<c:if test="${type_id=='5'}"><option value="4" <c:if test="${more_resource_type=='4'}">selected="selected"</c:if>>HTML5</option></c:if>
		    </select>
		    </li>
		    <div id="li_jishu">
		    	<li style="display:<c:if test="${type_id!='3'}">none</c:if>"><label>动画集数：<b>*</b></label>
	    			<input type="text" id="set_number" name="set_number" value="" maxlength="11"  class="dfinput" />
	    		</li>
	    	</div>
		    
		    <div id="li_dizhi">
		    	<li style="display:<c:if test="${type_id=='5'}">none</c:if>"><label>连接地址(app)：<b>*</b></label>
			    	<input type="text" id="resource_url_url" name="resource_url" value=""  class="dfinput" />
			    </li>
			    <li style="display:<c:if test="${type_id=='5'}">none</c:if>"><label>连接地址(分享)：<b>*</b></label>
			    	<input type="text" id="resource_url_url2" name="resource_url2" value=""  class="dfinput" />
			    </li>
		    </div>
		    <div id="li_wenjian">
		    <li style="display:<c:if test="${type_id!='4' && type_id!='1'}">none</c:if>"><label>资源音频文件：<b>*</b></label>
		    	<input type="text" id="resource_url_file" name="resource_url" value="" maxlength="150"  class="dfinput" readonly/>
			...
			<div id="li_html">
				<li style="display:<c:if test="${type_id!='5'}">none</c:if>">
					...
					<!-- 
		    		<textarea class="textinput700" cols="25" rows="2" name="resource_url_html" id="resource_url_html"></textarea>
		    		 -->
		    		 
		    		<input type="text" id="resource_url_html" name="resource_url_html" value="" maxlength="25"  class="dfinput" readonly/>
					<div style="padding-left:118px;">
		    ...
		    <div id="li_paixu">
			    <li style="display:<c:if test="${type_id!='3'}">none</c:if>"><label>排序：<b></b></label>
			    	<input type="text" id="resource_sort_cartoon" name="resource_sort_cartoon" value="" maxlength="11"  class="dfinput" />
			    </li>
		    </div>
		    <div id="li_geci">
			    <li style="display: <c:if test="${type_id!='1'}">none</c:if>"><label>歌词文件：<b></b></label>
			    	<input type="text" id="resource_lyrics" name="resource_lyrics" value="" maxlength="25"  class="dfinput" readonly/>
					<div style="padding-left:118px;">
		    ...
		    <li><label><b></b></label>
		    	...
		    	<input type="button" value=" 查 看 " name="close" id="close1" onclick="sel('${resource.id }');" class="btn"/>&nbsp;&nbsp;
		    	<span id="msg"></span></li>
