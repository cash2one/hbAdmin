...
<html>
   ...
   function addAdmin(){
		...
		$.ajax({
             type: "post",
             url: "<%=basePath %>globalLevel/update?callback=?",
             data: {
             	'id':id,
			...
			<div id="tab1" class="tabson">
				...
				<ul class="forminfo">
					<li>
						...
						<input type="text" id="level_content" name="level_content" size="30" value="${gl.level_content }"
							class="dfinput150" />
						<input type="hidden" id="index" name="index" size="30" value="${index }"
						class="dfinput150" />
						<input type="hidden" id="id" name="id" size="30" value="${gl.id }"
						class="dfinput150" />
					</li>
					<li>
						<label>
						...
						<input type="text" id="level_summary" name="level_summary" size="50" value="${gl.level_summary }"
							class="dfinput150" />
					</li>
					<li>
						<label>
						...
						<input type="text" id="sort" name="sort" size="8" value="${gl.sort }"
							class="dfinput150" />
					</li>
					<li>
						<label>
						...
						<select id="level_status" name="level_status" class="select select151">
							<option value="0" <c:if test="${gl.level_status=='0' }">selected</c:if>>
								不显示
							</option>
							<option value="1" <c:if test="${gl.level_status==1 }">selected</c:if>>
								显示
							</option>
							<option value="9" <c:if test="${gl.level_status==9 }">selected</c:if>>
								作废
							</option>
						</select>
					</li>
