...
<html>
  ...
  <script type="text/javascript">
			...
			function addrole(){
				...
				$.ajax({
		             type: "post",
		             url: "<%=basePath %>adminRole/doaddadminrole?callback=?",
		             data: {
		             		'adminAccount':admin,
		             		'roleName':rolename,
		             		'roleState':rolestate,
  ...
  <!-- 页面位置 -->
    ...
    <div class="itab">
	    ...
	    <li><a href="<%=basePath %>adminRole/toaddadminrole"  class="selected">角色添加</a></li> 
	  	</ul>
	</div> 
	
	<div id="tab1" class="tabson">
	    <ul class="forminfo">
	    	<li><label>角色名：<b>*</b></label><input type="text" id="rolename" name="rolename"  size="31" class="dfinput150"/></li>
		    	...
		    	<div style="min-height:200px;overflow-y:auto;"> 						
								<script type="text/javascript">
				   				 	...
				   				 	d.add("r_"+<%=ar0.getRightId()%>,'',
				   				 		"<input type='checkbox' id='<%=ar0.getRightId()%>' name='p0' style='zoom:100%' value='<%=ar0.getRightId()%>' onclick='checkSubAll(this);'/><font><%=ar0.getRightName()%></font>");
				   				 	<%
				   				 			}
				   				 		}
				   				 	%>
				   				 	...
				   				 	d.add("r1_"+<%=ar1.getRightId()%>,"r_"+<%=ar1.getParentId()%>,
				   				 		"<input type='checkbox' id='<%=ar1.getRightId()%>' name='p<%=ar1.getParentId()%>' style='zoom:100%' value='<%=ar1.getRightId()%>,<%=ar1.getParentId()%>'  onclick='checkSubAll(this);'/><font ><%=ar1.getRightName()%></font>");
				   				 	<%
				   				 			}
				   				 		}
				   				 	%>
				   				 	...
				   				 	d.add("r2_"+<%=ar2.getRightId()%>,"r1_"+<%=ar2.getParentId()%>,
				   				 		"<input type='checkbox' id='<%=ar2.getRightId()%>' name='p<%=ar2.getParentId()%>' style='zoom:100%' value='<%=ar2.getRightId()%>,<%=ar2.getParentId()%>' onclick='checkSubAll(this);' /><font ><%=ar2.getRightName()%></font>");
				   				 	<%
				   				 			}
				   				 		}
				   				 	%>
				   				 	...
				   				 	d.add("r3_"+<%=ar3.getRightId()%>,"r2_"+<%=ar3.getParentId()%>,
				   				 		"<input type='checkbox' id='<%=ar3.getRightId()%>' name='p<%=ar3.getParentId()%>' style='zoom:100%' value='<%=ar3.getRightId()%>,<%=ar3.getParentId()%>'  /><font ><%=ar3.getRightName()%></font>");
				   				 	<%
				   				 			}
				   				 		}
				   				 	%>
				   				 ...
				   				 <input type="hidden" id="strRight" name="strRight" value=""/>
				   				 <input type="hidden" id="strShcendRight" name="strShcendRight" value=""/>
		    ...
		    <li><label><b></b></label><input type="button" value=" 添  加 " name="close" id="close" onclick="javascript:toConfirmRights();addrole();" class="btn"/><span id="msg"></span></li>
